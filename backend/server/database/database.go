...
func Open(c *config.Config, dir string, local bool) (database *Database, err error) {
	var db *sql.DB
	if !local {
		...
		db, err = sql.Open("pgx", driverConfig.ConnectionString(c.Databases.Nick+"://"+c.Databases.Username+":"+c.Databases.Password+"@"+c.Databases.Host+":"+c.Databases.Port+"/"+c.Databases.Name))
		if err != nil {
			return nil, err
		}
	} else {
